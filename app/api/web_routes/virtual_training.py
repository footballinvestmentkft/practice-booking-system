...
@router.get("/virtual-training/stroop-challenge", response_class=HTMLResponse)
async def virtual_training_stroop_challenge(
    request: Request,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user_web),
):
    """Stroop Challenge game page — instructions + Vanilla JS game loop."""
    guard = require_student_onboarding(user)
    if guard:
        return guard

    game = VirtualTrainingService.get_game(db, "stroop_challenge")
    if game is None or not game.is_active:
        all_games = VirtualTrainingService.get_hub_games(db)
        return templates.TemplateResponse(
            "virtual_training_hub.html",
            {
                ...
                **_spec_ctx(user, db),
                "all_games": all_games,
                "error": "Stroop Challenge is not available at this time.",
            },
        )
    ...
    return templates.TemplateResponse(
        "virtual_training_stroop_challenge.html",
        {
            "request": request,
            "user": user,
            **_spec_ctx(user, db),
...
@router.post("/virtual-training/stroop-challenge/submit")
async def virtual_training_stroop_challenge_submit(
    request: Request,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user_web),
):
    """Record a Stroop Challenge attempt. Returns attempt_id, xp_awarded, is_valid."""
    guard = require_student_onboarding(user)
    if guard:
        return JSONResponse({"error": "onboarding required"}, status_code=403)
    ...
    started_at_raw = body.get("started_at", "")
    idem_key = f"vt_sc_u{user.id}_{started_at_raw}"
...
@router.get("/virtual-training/stroop-challenge/result/{attempt_id}",
            response_class=HTMLResponse)
async def virtual_training_stroop_challenge_result(
    attempt_id: int,
    request: Request,
    db: Session = Depends(get_db),
    user: User = Depends(get_current_user_web),
):
    """Result screen for a completed Stroop Challenge attempt."""
    guard = require_student_onboarding(user)
    if guard:
        return guard
    ...
    per_phase: list = []
    late_summary: dict | None = None
    conflict_cost_ms: int | None = None
    congruent_avg_rt_ms: int | None = None
    incongruent_avg_rt_ms: int | None = None
    raw = attempt.raw_metrics
    if isinstance(raw, dict) and raw.get("v", 1) >= 1:
        per_phase = raw.get("per_phase") or []
    if isinstance(raw, dict) and raw.get("v", 1) >= 2:
        late_summary = raw.get("late_summary") or None
    if isinstance(raw, dict) and int(raw.get("v", 1)) >= 3:
        conflict_cost_ms      = raw.get("conflict_cost_ms")
        congruent_avg_rt_ms   = raw.get("congruent_avg_rt_ms")
        incongruent_avg_rt_ms = raw.get("incongruent_avg_rt_ms")

    from ...models.user import UserRole
    is_admin = user.role == UserRole.ADMIN

    return templates.TemplateResponse(
        "virtual_training_stroop_challenge_result.html",
        {
            "request": request,
            "user": user,
            **_spec_ctx(user, db),
            ...
            "per_phase":     per_phase,
            "late_summary":  late_summary,
            "conflict_cost_ms":      conflict_cost_ms,
            "congruent_avg_rt_ms":   congruent_avg_rt_ms,
            "incongruent_avg_rt_ms": incongruent_avg_rt_ms,
            "is_admin":      is_admin,
        },
    )

...
# ── Call-site integration: N-01..N-05, P-01, P-02, inactive ──────────────────

class TestCallSiteEligibilityIntegration:
    """Mandatory negative/positive control tests for call-site binding.

    Accept-path tests exercise is_eligible_master_instructor directly
    (identical to what the accept endpoint calls after refactor).
    Planning-path tests use add_slot() with mock DB to verify 422 propagation.
    Field-path P-02 verifies is_eligible_field_instructor level calculus.
    """

    # ── Accept path ───────────────────────────────────────────────────────────

    def test_n01_accept_no_lfa_coach_license(self, postgres_db: Session):
        """N-01: instructor with no LFA_COACH license → not eligible."""
        instr = _make_instructor(postgres_db)
        ok, reason = is_eligible_master_instructor(postgres_db, instr.id, ["PRE"])
        assert ok is False
        assert "license" in reason.lower()

    def test_n02_accept_expired_license(self, postgres_db: Session):
        """N-02: expired LFA_COACH license → not eligible."""
    ...
    def test_n03_accept_insufficient_level(self, postgres_db: Session):
        """N-03: LFA_COACH level 5 for PRO tournament (requires ≥7) → not eligible."""
        instr = _make_instructor(postgres_db)
        _make_coach_license(postgres_db, instr, level=5)
        ok, reason = is_eligible_master_instructor(postgres_db, instr.id, ["PRO"])
        assert ok is False
        assert "insufficient" in reason.lower() or "level" in reason.lower()

    def test_inactive_license_not_eligible(self, postgres_db: Session):
        """Inactive license (is_active=False) rejected even when level is sufficient."""
        instr = _make_instructor(postgres_db)
        _make_coach_license(postgres_db, instr, level=8, is_active=False)
        ok, reason = is_eligible_master_instructor(postgres_db, instr.id, ["PRO"])
        assert ok is False
    ...
    def test_p01_accept_valid_license_sufficient_level(self, postgres_db: Session):
        """P-01: LFA_COACH L8 + PRO tournament → eligible."""
        instr = _make_instructor(postgres_db)
        _make_coach_license(postgres_db, instr, level=8)
        ok, reason = is_eligible_master_instructor(postgres_db, instr.id, ["PRO"])
        assert ok is True
        assert reason == ""

    # ── Planning path — add_slot() rejection (N-04, N-05) ────────────────────

    def test_n04_planning_master_ineligible_raises_422(self):
        """N-04: add_slot(MASTER) rejects ineligible instructor with 422."""
        from unittest.mock import MagicMock, patch
        from fastapi import HTTPException
        from app.services.tournament.instructor_planning_service import add_slot
        from app.models.tournament_instructor_slot import SlotRole

        tournament = MagicMock()
        tournament.id = 1
        tournament.age_group = "AMATEUR"
        tournament.age_groups = []
        tournament.tournament_config_obj = None
        ...
        def query_side(model):
            ...
            inner.first.return_value = tournament if call_count[0] == 1 else instructor
            return inner
        ...
        with patch(
            "app.services.tournament.instructor_planning_service.is_eligible_master_instructor",
            return_value=(False, "No active, valid LFA_COACH license found"),
        ):
            with pytest.raises(HTTPException) as exc_info:
                add_slot(db=db, semester_id=1, instructor_id=42,
                         role=SlotRole.MASTER.value, pitch_id=None, assigned_by_id=1)
        assert exc_info.value.status_code == 422
        assert "master" in exc_info.value.detail.lower()

    def test_n05_planning_field_ineligible_raises_422(self):
        """N-05: add_slot(FIELD) rejects instructor with insufficient level with 422."""
        from unittest.mock import MagicMock, patch
        from fastapi import HTTPException
        from app.services.tournament.instructor_planning_service import add_slot
        from app.models.tournament_instructor_slot import SlotRole
        ...
        with patch(
            "app.services.tournament.instructor_planning_service.is_eligible_field_instructor",
            return_value=(False, "Coach level 4 is insufficient for field role"),
        ):
            with pytest.raises(HTTPException) as exc_info:
                add_slot(db=db, semester_id=1, instructor_id=55,
                         role=SlotRole.FIELD.value, pitch_id=10, assigned_by_id=1)
        assert exc_info.value.status_code == 422

...
| `tricamera-capture-skeleton-proof` | implemented, **3-camera proof, requires manual operator step** | End-to-end 3-camera proof: iPhone (instructor + GoPro controller) + iPad (player) + GoPro (auxiliary). Runs one full capture cycle, then: (1) collects `capture_metadata_diag.json` from both iOS devices via `copy_app_container_file`; (2) runs Vision-based skeleton processing on iPhone's video and collects `skeleton_output.json`; (3) fetches GoPro media list as evidence; (4) collects `gopro_stream_diag.json` (GoPro preview UDP/decode evidence) and `pose_overlay_diag.json` (per-panel instructor/player/gopro frame-traffic counters, via the `pose-overlay-diag` deep link). **PASS is backend-grounded AND gated on this evidence** (2026-07-01 flow audit fixes): `instructor+player confirmed_start`, `gopro confirmed_start`, `all 3 confirmed_stop`, `timestamp sync report`, `gopro preview stream quality` (UDP packets + video PID + ≥1 decoded frame), `gopro preview aspect ratio is 16:9`, `{instructor,player,gopro} panel frame traffic` (≥1 frame reached each panel's pose overlay processor), and `{iphone,ipad} orientation consistent` + `effective aspect ratio is 16:9` (device orientation at recording start vs. the file's actual baked-in rotation). Skeleton-*point* detection and visual confirmation that skeletons are actually drawn on all 3 panels remain manual/visual — see PASS/FAIL definition below. Same GoPro WiFi manual join flow as other GoPro scenarios (operator gate in terminal). A physical run of this scenario MUST pass `python3 scripts/mc1_regression/preflight_static_check.py` first (`run_mc1_regression.sh` enforces this and refuses to start otherwise, short of an audited `SKIP_STATIC_PREFLIGHT=1` escape hatch that can never produce a valid PASS). |
